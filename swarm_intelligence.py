...
class Agent:
    ...
    def move(
        ...
        other: ndarray,
        rel=False,
        clamp=DomainClamping.STOP_ON_BOUNDARY,
    ) -> bool:
        """
        Переход в новую точку

        Относительный переход (rel == True) - к текущей координате прибавляется other
        Абсолютный переход (rel == False) - текущая координата приравнивается к other

        Возвращает, собирался ли агент остаться в пределах области определения
        """
